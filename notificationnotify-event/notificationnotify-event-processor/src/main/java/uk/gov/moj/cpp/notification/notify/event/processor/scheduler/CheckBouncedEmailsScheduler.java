...
@Singleton
@Startup
public class CheckBouncedEmailsScheduler {

    private static final Logger LOGGER = LoggerFactory.getLogger(CheckBouncedEmailsScheduler.class);
    private static final String TIMER_TIMEOUT_INFO = "CheckBouncedEmailsScheduler timer triggered.";

    @Inject
    @Value(key = "notification.notify.check.bounced.email.scheduler.interval", defaultValue = "120000")
    ...
    @Inject
    @Value(key = "notification_notify_mail_server_settings")
    ...
    public void startTimer() {
        LOGGER.info("CheckBouncedEmailsScheduler Triggers");
        mailServerSettings.forEach(mailServerSetting -> {
            final ExecutionInfo executionInfo = new ExecutionInfo((JsonObject) mailServerSetting, CHECK_BOUNCED_EMAILS.getTaskName(), utcClock.now(), STARTED, Priority.MEDIUM);
