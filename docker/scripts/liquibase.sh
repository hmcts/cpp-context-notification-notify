...
dbUserName=$2
dbPassword=$3
contextName=notificationnotify
dbPort=${5:-5432}
...
java -jar notificationnotify-viewstore-liquibase.jar --url=jdbc:postgresql://${dbServerName}:${dbPort}/${contextName}viewstore?sslmode=require --username=${dbUserName} --password=${dbPassword} --logLevel=info update
if [ $? -ne 0 ]
then
    exit 1
else
